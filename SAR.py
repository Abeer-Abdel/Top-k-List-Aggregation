...
# Run SAR and LS
print("\nSAR (Sort by Average Rank)")
key_SAR = lambda x: (avg_rank.get(x, math.inf),)
pre_groups = tie_groups_by_key(universe, key_SAR)
print("Full Ranking (With ties):", groups_to_string(pre_groups))
strict = random_tiebreak_sorted(universe, key_SAR, seed=SEED)[:k]
